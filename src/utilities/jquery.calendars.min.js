...
(function($) {
  ...
  $.extend(Calendars.prototype, {
    instance: function(a, b) {
      ...
      if (!c) {
        throw (
          this.local.invalidCalendar || this.regional[''].invalidCalendar
        ).replace(/\{0\}/, a);
      }
      return c;
    },
    ...
    compareTo: function(a) {
      if (this._calendar.name != a._calendar.name) {
        throw (
          ...
          $.calendars.regional[''].differentCalendars
        )
          .replace(/\{0\}/, this._calendar.local.name)
          .replace(/\{1\}/, a._calendar.local.name);
      }
      var c =
        this._year != a._year
          ? this._year - a._year
          : this._month != a._month
          ? this.monthOfYear() - a.monthOfYear()
          : this._day - a._day;
  ...
  $.extend(BaseCalendar.prototype, {
    ...
    monthOfYear: function(a, b) {
      ...
      return (
        ((c.month() + this.monthsInYear(c) - this.firstMonth) %
          this.monthsInYear(c)) +
        this.minMonth
      );
    },
    fromMonthOfYear: function(a, b) {
      var m =
        ((b + this.firstMonth - 2 * this.minMonth) % this.monthsInYear(a)) +
        this.minMonth;
      this._validate(
        a,
        m,
    ...
    dayOfYear: function(a, b, c) {
      ...
      return (
        ...
        this.newDate(
          d.year(),
          this.fromMonthOfYear(d.year(), this.minMonth),
          this.minDay
        ).toJD() +
        1
      );
    ...
    isValid: function(a, b, c) {
      ...
      if (d) {
        ...
        d =
          ...
          (c >= this.minDay && c - this.minDay < this.daysInMonth(e));
      }
      this._validateLevel--;
      return d;
    },
      ...
      if (a.year) {
        if (this._validateLevel == 0 && this.name != a.calendar().name) {
          throw (
            ...
            .replace(/\{1\}/, a.calendar().local.name);
        }
        return a;
      }
      try {
